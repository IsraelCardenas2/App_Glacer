...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const consumo_aguaInput = form.querySelector('input[name="Consumo_agua"]');
    ...
    const spanTextcalidad = form.querySelector('.span_calidad'); // Mensaje de error para campo calidad
    const spanTextconsumo_agua = form.querySelector('.span_consumo_agua'); // Mensaje de error para campo calidad
    ...
    const calidadRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
    const consumo_aguaRegex = /^[0-9]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
    ...
    form.addEventListener('submit', function(event) {
        ...
        const consumo_aguaValue = consumo_aguaInput.value;
        ...
        // Validar Consumo_agua
        if (!consumo_aguaRegex.test(consumo_aguaValue) || consumo_aguaValue === '') {
            event.preventDefault();
            spanTextconsumo_agua.style.color = 'rgba(224, 16, 16, 0.952)';
            spanTextconsumo_agua.style.display = 'inline';
            consumo_aguaInput.style.border = '2px solid rgba(224, 16, 16, 0.952)';
            isValid = false;
        } else {
            // Ocultar mensaje y resetear borde
            spanTextconsumo_agua.style.display = 'none';
            consumo_aguaInput.style.border = '';
        }
        // Validar marca
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upInodoro`);

        // Excalidades regulares para validación
        const nombreRegex = /^[A-Z][a-z]+?(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+)+$/;
        const codigoRegex = /^[A-Za-z0-9-]{1,15}$/;
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_medida, .span_color, .span_material, .span_modelo, .span_calidad, .span_consumo_agua');
            ...
            inputs.forEach(input => {
                ...
                switch (name) {
                    ...
                    case 'Calidad_act':
                        ...
                        break;
                    case 'Consumo_agua_act':
                        regex = consumo_aguaRegex;
                        span = form.querySelector('.span_consumo_agua');
                        break;
                }
