...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const spanTextcalidad = form.querySelector('.span_calidad'); // Mensaje de error para campo Acabado
    const spanTextcapacidad = form.querySelector('.span_capacidad'); // Mensaje de error para campo Acabado
    ...
    const capacidadRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,25}$/;
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upTinaco`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_medida, .span_color, .span_material, .span_calidad, .span_capacidad');
