...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const spanTextcontenido = form.querySelector('.span_contenido'); // Mensaje de error para campo Contenido
    ...
    const contenidoRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,39}$/;
    const acabadoRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,39}$/;
    const calidadRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,15}$/;
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upOff_PM`);

        // Exacabadoes regulares para validación
        const nombreRegex = /^[A-Z][a-z]+(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+)+$/;
        const codigoRegex = /^[A-Za-z0-9-]{1,15}$/;
        ...
        const marcaRegex = /^(?! )(?:[A-Z][a-záéíóúñÑ]+)(?: (?:[A-Z][a-záéíóúñÑ]+))?(?<! )$/;
        const materialRegex = /^[A-Z][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
        const acabadoRegex = /^(?! )(?:[A-Z][a-z]+)(?: (?:[A-Z][a-z]+))?(?<! )$/;
        const contenidoRegex = /^(?! )(?:[A-Z][a-z]+)(?: (?:[A-Z][a-z]+))?(?<! )$/;
