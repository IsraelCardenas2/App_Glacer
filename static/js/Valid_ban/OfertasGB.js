...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const precioantInput = form.querySelector('input[name="Precio_anterior"]');
    ...
    const detallesInput = form.querySelector('textarea[name="Detalles"]');
    ...
    const spanTextdetalles = form.querySelector('.span_detalles'); // Mensaje de error para campo Acabado
    ...
    const detallesRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,79}$/;
    ...
    form.addEventListener('submit', function(event) {
        ...
        const detallesValue = detallesInput.value;
        ...
        // Validar Detalles
        if (!detallesRegex.test(detallesValue) || detallesValue === '') {
            event.preventDefault(); // Prevenir envío

            // Mostrar mensaje y cambiar borde
            spanTextdetalles.style.color = 'rgba(224, 16, 16, 0.952)';
            spanTextdetalles.style.display = 'inline';
            detallesInput.style.border = '2px solid rgba(224, 16, 16, 0.952)';
            isValid = false;
        } else {
            // Ocultar mensaje y resetear borde
            spanTextdetalles.style.display = 'none';
            detallesInput.style.border = '';
        }
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upOffGB`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_medida, .span_color, .span_material, .span_detalles');
        ...
        form.addEventListener('submit', function(event) {
            ...
            inputs.forEach(input => {
                ...
                switch (name) {
                    ...
                    case 'Detalles_act':
                        regex = detallesRegex;
                        span = form.querySelector('.span_detalles');
                        break;
                }
