...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const tipo_colocacionInput = form.querySelector('input[name="Tipo_colocacion"]');
    ...
    const spanTexttipo_colocacion = form.querySelector('.span_tipo_colocacion'); // Mensaje de error para campo Acabado
    ...
    const tipo_colocacionRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
    ...
    form.addEventListener('submit', function(event) {
        ...
        const tipo_colocacionValue = tipo_colocacionInput.value;
        ...
        // Validar Tipo_colocacion
        if (!tipo_colocacionRegex.test(tipo_colocacionValue) || tipo_colocacionValue === '') {
            event.preventDefault(); // Prevenir envío

            // Mostrar mensaje y cambiar borde
            spanTexttipo_colocacion.style.color = 'rgba(224, 16, 16, 0.952)';
            spanTexttipo_colocacion.style.display = 'inline';
            tipo_colocacionInput.style.border = '2px solid rgba(224, 16, 16, 0.952)';
            isValid = false;
        } else {
            // Ocultar mensaje y resetear borde
            spanTexttipo_colocacion.style.display = 'none';
            tipo_colocacionInput.style.border = '';
        }
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upOvalin`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_medida, .span_color, .span_material, .span_tipo_colocacion');
            ...
            inputs.forEach(input => {
                ...
                switch (name) {
                    ...
                    case 'Material_act':
                        ...
                        break;
                    case 'Tipo_colocacion_act':
                        regex = tipo_colocacionRegex;
                        span = form.querySelector('.span_tipo_colocacion');
                        break;
                }
