...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const tipo_installInput = form.querySelector('input[name="Tipo_install"]');
    ...
    const spanTexttipo_install = form.querySelector('.span_tipo_install'); // Mensaje de error para el campo Tipo_install
    ...
    const precioRegex = /^(?!0(\.0{1,2})?$)[1-9][0-9]{2,}(\.[0-9]{1,2})?$/; // Solo números y opcionalmente un decimal
    const precioantRegex = /^(?!0(\.0{1,2})?$)[1-9][0-9]{2,}(\.[0-9]{1,2})?$/; // Solo números y opcionalmente un decimal
    const materialRegex = /^[A-Z][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
    const tipo_installRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,59}$/;
    ...
    form.addEventListener('submit', function(event) {
        ...
        const tipo_installValue = tipo_installInput.value;
        ...
        // Validar tipo_install
        if (!tipo_installRegex.test(tipo_installValue) || tipo_installValue === '') {
            event.preventDefault();
            spanTexttipo_install.style.color = 'rgba(224, 16, 16, 0.952)';
            spanTexttipo_install.style.display = 'inline';
            tipo_installInput.style.border = '2px solid rgba(224, 16, 16, 0.952)';
            isValid = false;
        } else {
            spanTexttipo_install.style.display = 'none';
            tipo_installInput.style.border = '';
        }
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upManeral`);

        // Expresiones regulares para validación
        const nombreRegex = /^[A-Z][a-z]+(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+)+$/;
        const precioRegex = /^(?!0(\.0{1,2})?$)[1-9][0-9]{2,}(\.[0-9]{1,2})?$/;
        const precioantRegex = /^(?!0(\.0{1,2})?$)[1-9][0-9]{2,}(\.[0-9]{1,2})?$/;
        const tipo_installRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,59}$/;
        const materialRegex = /^[A-Z][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Za-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
        const marcaRegex = /^(?! )(?:[A-Z][a-záéíóúñÑ]+)(?: (?:[A-Z][a-záéíóúñÑ]+))?(?<! )$/;
        const codigoRegex = /^[A-Za-z0-9-]{1,15}$/;
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_tipo_install, .span_color, .span_material');
            ...
            inputs.forEach(input => {
                ...
                switch (name) {
                    ...
                    case 'Marca_act':
                        ...
                        break;
                    case 'Tipo_install_act':
                        regex = tipo_installRegex;
                        span = form.querySelector('.span_tipo_install');
                        break;
                    case 'Color_act':
                        regex = colorRegex;
