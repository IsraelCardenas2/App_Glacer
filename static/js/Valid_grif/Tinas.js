...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const spanTextcapacidad = form.querySelector('.span_capacidad'); // Mensaje de error para campo color
    ...
    const spanTexttipo = form.querySelector('.span_tipo'); // Mensaje de error para campo Flujo_agua
    ...
    const complementosRegex = /^(?! )(?:[A-Z][a-z]+)(?: (?:[A-Z][a-z]+))?(?<! )$/;
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upTina`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_medida, .span_color, .span_capacidad, .span_material, .span_tipo, .span_complementos');
