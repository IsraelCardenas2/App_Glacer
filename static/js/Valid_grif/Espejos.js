...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const complementoRegex = /^[A-Z][a-z]+(?:[,.]?[ ]?[a-z]+)+$/;
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upEspejo`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_medida, .span_color, .span_material, .span_complementos');
