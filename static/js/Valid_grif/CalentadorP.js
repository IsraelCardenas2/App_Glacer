...
document.addEventListener("DOMContentLoaded", function() {
    ...
    const servicioInput = form.querySelector('input[name="Servicio"]');
    ...
    const spanTextservicio = form.querySelector('.span_servicio'); // Mensaje de error para campo Servicio
    ...
    const servicioRegex = /^[A-Z0-9][a-záéíóúÁÉÍÓÚñÑ]+(\s[A-Z0-9a-záéíóúÁÉÍÓÚñÑ]+){0,49}$/;
    ...
    form.addEventListener('submit', function(event) {
        ...
        // Validar Servicio
        if (!servicioRegex.test(servicioValue) || servicioValue === '') {
            event.preventDefault();
            spanTextservicio.style.color = 'rgba(224, 16, 16, 0.952)';
            spanTextservicio.style.display = 'inline';
            servicioInput.style.border = '2px solid rgba(224, 16, 16, 0.952)';
            isValid = false;
        } else {
            // Ocultar mensaje y resetear borde
            spanTextservicio.style.display = 'none';
            servicioInput.style.border = '';
        }
...
document.querySelectorAll('[id^="btn_actualizar"]').forEach(button => {
    button.addEventListener("click", function(event) {
        ...
        const form = document.querySelector(`${modalId} .upCalentadorP`);
        ...
        const spans = form.querySelectorAll('.span_text, .span_codigo, .span_precio, .span_PrecioAnt, .span_marca, .span_color, .span_material, .span_capacidad, .span_servicio');
        ...
        form.addEventListener('submit', function(event) {
            ...
            inputs.forEach(input => {
                ...
                switch (name) {
                    ...
                    case 'Servicio_act':
                        regex = servicioRegex;
                        span = form.querySelector('.span_servicio');
                        break;
                }
